...
**v0.4.0**: Wizard `/login` dengan **tes koneksi live** + pesan error yang menerjemahkan, **multi-profil provider** (`/profile hemat|kuat|lokal`) + auto-fetch daftar model, **role AI berlapis** (`/role`, AGENT.md, `/mode beginner|pro`), **plan mode** & deteksi loop ditegakkan di kode, tool `patch_file` hemat token, `/undo` snapshot file, `/compact`, dan baris usage `↑ ↓ · ctx%` tiap giliran. (v0.3.0: setup wizard, streaming SSE, status bar, diff visual, distribusi global.)
...
Jalankan `ruko` pertama kali tanpa API key → wizard interaktif muncul berturut-turut: banner *Welcome* berlatar biru, lalu prompt `API Key:` → `Base URL (default https://api.b.ai/v1):` → `Model Name (default qwen3.8-flash):`. Sebelum disimpan, CLI **menguji koneksi live** dan menampilkan `✓ Terhubung ke <model>` — atau pesan error yang *menerjemahkan* (401 → "API key salah atau kedaluwarsa", 404 → "Model tidak ditemukan", ECONNREFUSED localhost → "Server lokal belum jalan (`ollama serve`)") lengkap dengan perintah perbaikan + opsi coba ulang/simpan paksa/batal. Semuanya tersimpan permanen ke `.ruko/config.json` dengan izin **600** — tanpa export manual. Kapan pun bisa diulang dari REPL dengan `/login` (atau `/config setup`).
